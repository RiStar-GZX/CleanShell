#include "lingfixed.h"

typedef struct
{
    GtkLayoutManager * layout_manager;
    GList * items_list;
}LingFixedPrivate;
G_DEFINE_TYPE_WITH_PRIVATE (LingFixed, ling_fixed, GTK_TYPE_FIXED)


struct _LingFixedLayout{
    GtkLayoutManager parent;
};

G_DEFINE_FINAL_TYPE(LingFixedLayout,ling_fixed_layout,GTK_TYPE_LAYOUT_MANAGER)

static void ling_fixed_layout_allocate(GtkLayoutManager *manager,GtkWidget *widget,
                int width,int height,int baseline){
    GtkWidget * parent = gtk_widget_get_parent(widget);
    if(parent==NULL)return;

    graphene_rect_t rect;
    if(!gtk_widget_compute_bounds(parent,widget,&rect))return;
    int start = gtk_widget_get_margin_start(widget);
    int end = gtk_widget_get_margin_end(widget);
    int top = gtk_widget_get_margin_top(widget);
    int bottom = gtk_widget_get_margin_bottom(widget);
    rect.size.height-=top+bottom;
    rect.size.width-=start+end;
    LingFixed * fixed = LING_FIXED(widget);
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    for(GList * l=self->items_list;l!=NULL;l=l->next){
        LingFixedItem * item = l->data;
        GtkAllocation child_allocation;
        if(item->adjust!=NULL){
            child_allocation = item->adjust(widget,item,width,height,baseline,rect);
            if(child_allocation.width<=0)child_allocation.width=width;
            if(child_allocation.height<=0)child_allocation.height=height;

            gtk_widget_size_allocate(item->widget, &child_allocation, baseline);
        }
    }
}

static void ling_fixed_layout_measure(GtkLayoutManager *manager,GtkWidget *widget,
                GtkOrientation orientation,int for_size,
                int *minimum,int *natural,
                int *minimum_baseline,int *natural_baseline){
    GtkAllocation alloc;
    gtk_widget_get_allocation(widget, &alloc);
    //设置子部件尺寸为容器尺寸
    *minimum = (orientation == GTK_ORIENTATION_HORIZONTAL) ? alloc.width : alloc.height;
    *natural = *minimum;
}

void ling_fixed_layout_class_init(LingFixedLayoutClass * klass){
    GtkLayoutManagerClass * manager_class = GTK_LAYOUT_MANAGER_CLASS(klass);
    manager_class->allocate = ling_fixed_layout_allocate;
    //manager_class->measure = ling_fixed_layout_measure;
}

void ling_fixed_layout_init(LingFixedLayout * self){
}

GtkLayoutManager * ling_fixed_layout_new(){
    return g_object_new(LING_TYPE_FIXED_LAYOUT,NULL);
}

// gboolean ling_fixed_translate_coordinates(GtkWidget  *src_widget,
//                                           LingFixed  *fixed,
//                                           GtkWidget  *dest_widget,
//                                           double      src_x,
//                                           double      src_y,
//                                           double     *dest_x,
//                                           double     *dest_y){    //暂时方案
//     double x,y;
//     if(!gtk_widget_translate_coordinates(src_widget,GTK_WIDGET(fixed),src_x,src_y,&x,&y))return FALSE;
//     LingFixedItemInfo * info = ling_fixed_get_item_info(fixed,dest_widget);
//     if(info==NULL)return FALSE;
//     x+=info->x;
//     y+=info->y;
//     *dest_x=x;
//     *dest_y=y;
//     return TRUE;
// }

/***************************fixed****************************************/



//G_DEFINE_FINAL_TYPE(LingFixed,ling_fixed,GTK_TYPE_FIXED)

void ling_fixed_class_init(LingFixedClass * self){
}

void ling_fixed_init(LingFixed * fixed){
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    self->items_list = NULL;
    self->layout_manager = ling_fixed_layout_new();
    gtk_widget_set_layout_manager(GTK_WIDGET(fixed),self->layout_manager);
}

GtkWidget * ling_fixed_new(){
    return g_object_new(LING_TYPE_FIXED,NULL);
}

LingFixedItem * ling_fixed_get_item_info(LingFixed * fixed,GtkWidget * widget){
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    for(GList * l=self->items_list;l!=NULL;l=l->next){
        LingFixedItem * item = l->data;
        if(item->widget==widget){
            return item;
        }
    }
    return NULL;
}

void ling_fixed_set_child_size(LingFixed * fixed,GtkWidget * widget,int w,int h){
    LingFixedItem * item=ling_fixed_get_item_info(fixed,widget);
    if(item==NULL)return;
    if(w<0)w=0;
    if(h<0)h=0;
    item->w = w;
    item->h = h;
    // if(gtk_widget_get_realized(widget)){
    //     GtkWidget * parent = gtk_widget_get_parent(item->widget);
    //     if(parent==NULL)return;
    //     GtkLayoutManager * layout = gtk_widget_get_layout_manager(parent);
    //     if(layout==NULL)return;
    //     gtk_layout_manager_allocate(layout,parent,w,h,-1);
    // }
    gtk_widget_set_size_request(widget,w,h);
}


void ling_fixed_move(LingFixed * fixed,GtkWidget * widget,gdouble x,gdouble y){
    LingFixedItem * item=ling_fixed_get_item_info(fixed,widget);
    if(item==NULL)return;
    item->x = x;
    item->y = y;
    if(gtk_widget_get_realized(widget)){
        GtkWidget * parent = gtk_widget_get_parent(item->widget);
        if(parent==NULL)return;
        GtkLayoutManager * layout = gtk_widget_get_layout_manager(GTK_WIDGET(fixed));
        if(layout==NULL)return;
        gtk_layout_manager_allocate(layout,parent,item->w,item->h,-1);
    }
}

gpointer ling_fixed_remove(LingFixed * fixed,GtkWidget * widget){
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    LingFixedItem * item;
    for(GList * l=self->items_list;l!=NULL;l=l->next){
        item = l->data;
        if(item->widget==widget){
            GtkWidget * parent = gtk_widget_get_parent(item->widget);
            if(parent==NULL)return NULL;
            GtkLayoutManager * layout = gtk_widget_get_layout_manager(GTK_WIDGET(fixed));
            if(layout==NULL)return NULL;
            g_object_ref(widget);
            gtk_widget_unparent(widget);
            self->items_list = g_list_remove_link(self->items_list,l);
            free(item);
            //gtk_layout_manager_allocate(layout,parent,item->w,item->h,-1);
            return item->ex;
        }
    }
    return NULL;
}

void ling_fixed_set_child_level(LingFixed * fixed,GtkWidget * widget,int level1,int level2){
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    LingFixedItem * item = ling_fixed_get_item_info(fixed,widget);
    if(item==NULL)return;
    if(level1>=0)item->level1 = level1;
    if(level2>=0)item->level2 = level2;
    for(GList * l = self->items_list;l!=NULL;l=l->next){
        LingFixedItem * item_now = (LingFixedItem *)l->data;
        if(item_now->level1>level1);
        else if(item_now->level1==level1){
            if(item_now->level2>level2);
            else{
                gtk_widget_insert_before(widget,GTK_WIDGET(fixed),item_now->widget);
                self->items_list = g_list_insert_before(self->items_list,l,item);
                return;
            }
        }
        else{
            gtk_widget_insert_before(widget,GTK_WIDGET(fixed),item_now->widget);
            self->items_list = g_list_insert_before(self->items_list,l,item);
            return;
        }
    }
}



LingFixedItem * ling_fixed_put(LingFixed * fixed,GtkWidget * widget,LING_FIXED_ADJUST adjust,uint x,uint y,uint level1,uint level2){
    LingFixedPrivate * self =ling_fixed_get_instance_private(fixed);
    LingFixedItem * item=ling_fixed_get_item_info(fixed,widget);
    //没有child创建
    if(item==NULL){
        item = g_malloc0(sizeof(LingFixedItem));
        item->widget = widget;
        item->level1 = level1;
        item->level2 = level2;
        item->w = -1;  //未分配长度
        item->h = -1;
        item->x = x;
        item->y = y;
        item->adjust = adjust;

        //gtk_widget_set_parent(widget,GTK_WIDGET(fixed));
        //gtk_widget_set_layout_manager(item->widget,GTK_LAYOUT_MANAGER(item));
    }

    //添加
    self->items_list = g_list_remove(self->items_list,item);
    for(GList * l = self->items_list;l!=NULL;l=l->next){
        LingFixedItem * item_now = (LingFixedItem *)l->data;
        if(item_now->level1>level1);
        else if(item_now->level1==level1){
            if(item_now->level2>level2);
            else{
                gtk_widget_insert_before(widget,GTK_WIDGET(fixed),item_now->widget);
                self->items_list = g_list_insert_before(self->items_list,l,item);
                return item;
            }
        }
        else{
            gtk_widget_insert_before(widget,GTK_WIDGET(fixed),item_now->widget);
            self->items_list = g_list_insert_before(self->items_list,l,item);
            return item;
        }
    }
    gtk_widget_insert_after(widget,GTK_WIDGET(fixed),gtk_widget_get_last_child(GTK_WIDGET(fixed)));
    self->items_list = g_list_append(self->items_list,item);
    return item;
}

GtkAllocation ling_fixed_adjust_none(GtkWidget * fixed,LingFixedItem *item,
                                     int width,int height,int baseline,graphene_rect_t rect){
    GtkAllocation alloc;
    alloc.width=item->w;
    alloc.height=item->h;
    alloc.x=item->x;
    alloc.y=item->y;
    return alloc;
}

LingFixedItem * ling_fixed_put_none(LingFixed * fixed,GtkWidget * widget,uint x,uint y,uint level1,uint level2){
    return ling_fixed_put(fixed,widget,ling_fixed_adjust_none,x,y,level1,level2);
}

GtkAllocation ling_fixed_adjust_full(GtkWidget * fixed,LingFixedItem *item,
                                     int width,int height,int baseline,graphene_rect_t rect){
    GtkAllocation alloc;
    alloc.width=rect.size.width;
    alloc.height=rect.size.height;
    alloc.x=item->x;
    alloc.y=item->y;
    return alloc;
}

LingFixedItem * ling_fixed_put_full(LingFixed * fixed,GtkWidget * widget,uint x,uint y,uint level1,uint level2){
    return ling_fixed_put(fixed,widget,ling_fixed_adjust_full,x,y,level1,level2);
}

void ling_fixed_item_change_adjust(LingFixed * fixed,GtkWidget * widget,LING_FIXED_ADJUST adjust){
    LingFixedItem * item=ling_fixed_get_item_info(fixed,widget);
    item->adjust = adjust;
    GtkLayoutManager * layout = gtk_widget_get_layout_manager(GTK_WIDGET(fixed));
    if(layout==NULL)return;
    gtk_layout_manager_allocate(layout,GTK_WIDGET(fixed),item->w,item->h,-1);
}

GList * ling_fixed_get_items_list(LingFixed * fixed){
    LingFixedPrivate * self = ling_fixed_get_instance_private(fixed);
    return self->items_list;
}
