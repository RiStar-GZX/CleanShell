...
GtkWidget * cl_control_center_new(GtkWidget * overlay){
...
void cl_control_center_load(ClControlCenter * self){
    self->wlan_button = cl_control_button_new("wlan","network-wireless-100");
    ling_grid_attach(LING_GRID(self->grid),self->wlan_button,1,1,2,1);

    GtkWidget * folder= cl_wlan_list_new();
    ...
    self->ble_button = cl_control_button_new("蓝牙","bluetooth-symbolic");
    ling_grid_attach(LING_GRID(self->grid),self->ble_button,3,1,2,1);

    GtkWidget * ble_folder= cl_ble_list_new();
    ...
    ling_grid_attach(LING_GRID(self->grid),self->brightness,3,2,1,2);
    ...
    ling_grid_attach(LING_GRID(self->grid),self->volume,4,2,1,2);
