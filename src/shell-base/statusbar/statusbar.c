...
G_DEFINE_FINAL_TYPE(ClCenterHead,cl_center_head,GTK_TYPE_BOX);

static gboolean ling_center_head_status_update(gpointer user_data){
    ClCenterHead * self = CL_CENTER_HEAD(user_data);
...
static void cl_center_head_class_init(ClCenterHeadClass * klass){

}

static void cl_center_head_init(ClCenterHead * self){
...
GtkWidget * cl_center_head_new(){
    return g_object_new(CL_TYPE_CENTER_HEAD,NULL);
}
...
G_DEFINE_FINAL_TYPE(ClStatusBar,cl_status_bar,GTK_TYPE_BOX);

void cl_status_bar_set_time(ClStatusBar * self);
gboolean cl_status_bar_get_battery_capacity(ClStatusBar * self);
gboolean cl_status_bar_get_wlan_quality(ClStatusBar * self,const char * interface);
int cl_status_bar_set_css_color(ClStatusBar * self);


static gboolean update_status(gpointer user_data){
    ClStatusBar * self = (ClStatusBar*)user_data;
...
static void cl_status_bar_init(ClStatusBar * self){
    ...
    cl_status_bar_set_css_color(self);
...
GtkWidget * cl_status_bar_new(){
    ClStatusBar * self = CL_STATUS_BAR(g_object_new(CL_TYPE_STATUS_BAR,NULL));
    cl_status_bar_setting(self);
