...
G_DEFINE_FINAL_TYPE(ClLockScreen,cl_lock_screen,GTK_TYPE_BOX);

void cl_lock_screen_class_init(ClLockScreenClass * klass){
...
void cl_lock_screen_init(ClLockScreen * self){
...
GtkWidget * cl_lock_screen_new(){
    ClLockScreen * self = CL_LOCK_SCREEN(g_object_new(CL_TYPE_LOCK_SCREEN,NULL));
    strcpy(self->passwd,"");
