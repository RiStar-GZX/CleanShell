...
struct _LingTaskPage{
    ...
    GtkWidget * app;
    LingTaskSwitcher * task_switcher;
};

G_DEFINE_FINAL_TYPE(LingTaskPage,ling_task_page,GTK_TYPE_BOX);
...
static void ling_task_page_class_init(LingTaskPageClass * klass){

}

static gdouble task_page_progress(GtkWidget * widget,LingActionArgs args,gpointer user_data){

    LingTaskPage * page = LING_TASK_PAGE(widget);
...
static void  task_page_finish_e(GtkWidget * widget,LingActionArgs args,gpointer user_data){
    LingTaskPage * task_page = user_data;
    LingTaskSwitcher * ts = task_page->task_switcher;
...
static void ling_task_page_init(LingTaskPage * self){
...
GtkWidget * ling_task_page_new(LingTaskSwitcher * task_switcher){
    LingTaskPage * self = g_object_new(LING_TYPE_TASK_PAGE,NULL);
...
static void finish_s(GtkWidget * widget,LingActionArgs args,gpointer user_data){
    LingTaskSwitcher * self = user_data;
    gtk_widget_set_visible(GTK_WIDGET(self),FALSE);
    LingLayer * layer;
    ling_desktop_get_layer_bodybox(LING_DESKTOP(shell->desktop),&layer);
...
static void add_page_clicked (GtkButton* button,gpointer user_data){
    LingTaskSwitcher * self = user_data;
    self->button_clicked = TRUE;
    ling_view_pager_add_page(LING_VIEW_PAGER(self->viewpager),ling_task_page_new(self));
